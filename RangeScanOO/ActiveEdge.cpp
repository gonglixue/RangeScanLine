...
ActiveEdge::ActiveEdge(Edge* e1, Edge* e2)
{
	...
	if (fabs(e1->x_ - e2->x_) < 0.00001 && fabs(e1->y_up - e2->y_up)<0.00001)
	{
		if (e1->dx_ < e2->dx_)
		{
			xl_ = e1->x_;
...
ActiveEdge::ActiveEdge(Edge* e1, Edge* e2, int scan_y)
{
	...
	//else {
		...
		if (cur_x_e1 < cur_x_e2)  // e1为左
		{
			...
			xr_ = cur_x_e2;
			dxr_ = e2->dx_;
			dyr_ = e2->dy_ - (scan_y - int(e2->y_up+0.5));
			...
			xl_ = cur_x_e2;
			dxl_ = e2->dx_;
			dyl_ = e2->dy_ - (scan_y - int(e2->y_up+0.5));

			xr_ = cur_x_e1;
			dxr_ = e1->dx_;
			dyr_ = e1->dy_ - (scan_y - int(e1->y_up+0.5));
	...
	//}
		...
		if (dxl_ > 0 && xl_ > edge_l_->x_down_)
			xl_ = edge_l_->x_down_;
		else if (dxl_ < 0 && xl_ < edge_l_->x_down_)
			xl_ = edge_l_->x_down_;

		if (dxr_ > 0 && xr_ > edge_r_->x_down_)
			xr_ = edge_r_->x_down_;
		else if (dxr_ < 0 && xr_ < edge_r_->x_down_)
			xr_ = edge_r_->x_down_;
	...
	zl_ = edge_l_->z_up_ + dzx_ * (xl_ - edge_l_->x_) + dzy_ * (scan_y - int(edge_l_->y_up+0.5)); //?
...
ActiveEdge::ActiveEdge(ActiveEdge* update_from_ae)
{
	...
	// 需要考虑当斜率接近于0，dx超级大的情况？？
	if (dxl_ > 0 && xl_ > update_from_ae->edge_l_->x_down_)
		xl_ = update_from_ae->edge_l_->x_down_;
	else if (dxl_ < 0 && xl_ < update_from_ae->edge_l_->x_down_)
		xl_ = update_from_ae->edge_l_->x_down_;

	if (dxr_ > 0 && xr_ > update_from_ae->edge_r_->x_down_)
		xr_ = update_from_ae->edge_r_->x_down_;
	else if (dxr_ < 0 && xr_ < update_from_ae->edge_r_->x_down_)
		xr_ = update_from_ae->edge_r_->x_down_;
	...
	edge_l_ = update_from_ae->edge_l_;
	edge_r_ = update_from_ae->edge_r_;
